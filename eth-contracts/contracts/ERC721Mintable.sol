...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender==_owner, "This function can be only called by the owner of the contract");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner!=address(0),"Please use a valid address");
        address oldOwner = _owner;
        _owner = newOwner;
        emit ownerTransference(oldOwner,newOwner);
    }

    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event ownerTransference(address oldOwner, address newOwner);
}
...
//  2) create a public setter using the inherited onlyOwner modifier
    function pause(bool pauseState) public onlyOwner {
        _paused = pauseState;
        if (_paused) emit pauseEvent(msg.sender); else emit unpauseEvent(msg.sender);
    }
...
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation
    modifier whenNotPaused() {
        require(!_paused,"Contract need to be unpaused");
        _;
    }
    modifier paused() {
        require(_paused,"Contract need to be paused");
        _;
    }
    //  5) create a Paused & Unpaused event that emits the address that triggered the event
    event pauseEvent(address emitter);
    event unpauseEvent(address emitter);
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(_tokenOwner[tokenId]!=to, "This address already owns the token");
        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(_tokenOwner[tokenId]==msg.sender||isApprovedForAll(_tokenOwner[tokenId],to), "This approval operation is not allowed");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId]==from,"The address provided is not the owner of the token");
        // require token is being transfered to valid address
        require(to!=address(0),"Please provide a valid address");
        // clear approval
...
contract TheCoolestToken is ERC721Metadata("The Coolest Token", "TCT", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){
